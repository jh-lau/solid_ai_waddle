...
class BaseTrainer:
    ...
    def _log_csv(self, dataset_label: str, data_label: str, *data: Union[float, int]):
    ...
    def _save_best(self, model: PreTrainedModel, tokenizer: PreTrainedTokenizer, optimizer: Optimizer,
        ...
        if accuracy > self._best_results[label]:
            self._logger.info("[%s] Best models in iteration %s: %s%% accuracy" % (label, iteration, accuracy))
